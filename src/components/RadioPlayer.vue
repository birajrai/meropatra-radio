...
const { playlist, playing, playingError, playingIndex, playingStation, togglePlay, prev, next } = usePlayer(stations);
    ...
    <div class="radio-player">
        <main>
          ...
          <button class="playing-actions--favorite">
            ...
            <div class="controls">
                <button class="controls--prev" :disabled="!allowPrev" @click="prev()">
                    <material-icon name="skip_previous" type="rounded" size="32" />
                </button>
                <button class="controls--play" @click="togglePlay()">
                    <material-icon :name="playing ? 'stop' : 'play_arrow'" type="rounded" size="32" />
                </button>
                <button class="controls--prev" :disabled="!allowNext" @click="next()">
                    <material-icon name="skip_next" type="rounded" size="32" />
                </button>
            </div>
        </main>
        <div class="playlist">
